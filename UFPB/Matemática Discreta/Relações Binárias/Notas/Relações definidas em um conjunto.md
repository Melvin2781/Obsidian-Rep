...
### Fecho reflexivo:
	Você adiciona pares ordenados até obter uma relação reflexiva, você pode fazer isso unindo a diagonal de A(x,x) com a relação original. Também chamada de identidade de A.
...
### Fecho Transitivo:
	O fecho transitivo de uma relação binária R em um conjunto A é a menor relação transitiva, em A que contém R. O fecho transitivo é obtido pelas uniões sucessivas de R^i, com i partindo de i e podendo chegar até o infinito. Lembrando da propriedade desse fecho com relação à cardinalidade do conjunto. Nós iremos unir até |A| = K, fazendo o somatório da união de 1 até k.
